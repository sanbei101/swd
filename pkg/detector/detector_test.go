...
func TestDetector_Detect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			d, err := NewDetector(tt.options, loader, algo)
			if err != nil {
...
func TestDetector_MaxDistance(t *testing.T) {
	loader := dictionary.NewLoader()
	_ = loader.LoadDefaultWords(context.Background())
	algo := algorithm.NewAhoCorasick()
	d, err := NewDetector(core.SWDOptions{IgnoreCase: true, MaxDistance: 2}, loader, algo)
	if err != nil {
		t.Fatalf("创建检测器失败: %v", err)
	}
	if !d.Detect("f*u*c*k") {
		t.Errorf("MaxDistance Detect() 期望命中")
	}
}

func TestDetector_URL_Email(t *testing.T) {
	loader := dictionary.NewLoader()
	_ = loader.LoadDefaultWords(context.Background())
	algo := algorithm.NewAhoCorasick()
	d, err := NewDetector(core.SWDOptions{EnableURLCheck: true, EnableEmailCheck: true}, loader, algo)
	if err != nil {
		t.Fatalf("创建检测器失败: %v", err)
	}
	if !d.Detect("访问 https://example.com/path?a=1") {
		t.Errorf("URL Detect() 期望命中")
	}
	if !d.Detect("联系 test.user+1@example.org") {
		t.Errorf("Email Detect() 期望命中")
	}
}
